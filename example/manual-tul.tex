...
\documentclass[a4paper,12pt,fonts,numbering,listingsleft]{./tulpackage/tularticle}
...
% Konfiguraci můžete změnit v hlavičce tohoto souboru (tul / zotero)
% autolang=none brání změnám jazyka strukturálních řetězců (dostupné z X available from) podle jazyka zdroje
\usepackage[
  backend=biber,
...
\hypersetup{pdftitle={Návod k použití balíku tul pro LaTeX (\verze)}}
\makeindex
...
\TULfancytitlepage{Návod k~použití balíku tul pro \LaTeX\ (\verze)}
{Pavel Satrapa}
{Liberec~2024}
...
nástrojů pro~správu fontů a~jazyků je vyžadován překlad pomocí \XeLaTeX{}u nebo\linebreak
\LuaLaTeX{}u (klasický pdf\LaTeX{} není podporován).

Seznam využitých balíků:

\begin{itemize}[label={}]
    \item \cmdfont{caption}~-- formátování popisků obrázků a~tabulek
    \item \cmdfont{etoolbox}~-- programátorské nástroje a~patchování maker
    \item \cmdfont{fancyhdr}~-- nastavení záhlaví a~zápatí stránky
    \item \cmdfont{float}~-- definice a~umístění plovoucích objektů
    \item \cmdfont{fontspec}~-- načítání a~správa systémových fontů
    \item \cmdfont{framed}~-- tvorba orámovaných a~podbarvených bloků
    \item \cmdfont{fvextra}~-- pokročilé formátování verbatim textu
    \item \cmdfont{parskip}~-- nastavení výchozího odstupu mezi odstavci
    \item \cmdfont{graphicx}~-- vkládání a~manipulace s~obrázky
    \item \cmdfont{hyperref}~-- podpora interaktivních odkazů a~metadat
    \item \cmdfont{ifthen}~-- podmíněné příkazy a~logické struktury
    \item \cmdfont{makeidx}~-- nástroje pro~tvorbu rejstříku
    \item \cmdfont{metalogo}~-- loga \TeX{}ových systémů a~maker
    \item \cmdfont{pdfpages}~-- vkládání externích PDF dokumentů
    \item \cmdfont{polyglossia}~-- moderní podpora jazyků a~dělení slov
    \item \cmdfont{silence}~-- filtrování a~potlačení varovných hlášení
    \item \cmdfont{tabularray}~-- sazba komplexních tabulek s~lepším rozhraním
    \item \cmdfont{tabularx}~-- tabulky s~automatickým zalamováním sloupců
    \item \cmdfont{booktabs}~-- stylizace tabulek pro~lepší čitelnost
    \item \cmdfont{titlesec}~-- úprava vzhledu nadpisů sekcí
    \item \cmdfont{enumitem}~-- úprava vzhledu číslovaných seznamů
    \item \cmdfont{tocloft}~-- formátování obsahu a~seznamů obrázků
    \item \cmdfont{xcolor}~-- definice a~míchání barev
\end{itemize}
...
která přímo vytváří PDF výstup, jako je~například \XeLaTeX\ nebo Lua\LaTeX.
...
vizuálním stylu celé univerzity. Jsou k~dispozici tyto
\textbf{volby součástí TUL}: \cmdfont{FS}, \cmdfont{FT}, \cmdfont{FP},
\cmdfont{EF}, \cmdfont{FUA}, \cmdfont{FM}, \cmdfont{FZS}, \cmdfont{CXI}.
Použití některé z~nich změní výchozí barvu, logo a~styl zápatí stránek.
Používejte vždy nanejvýš jednu z~těchto voleb.
...
\begin{itemize}[label={}, leftmargin=0pt]
    \raggedright%
    \item \cmdfont{EN}~-- je aktivována anglická verze, což se týká generických příkazů
          (např.~\cmd{logo}) a~nápisů v~zápatí
    \item \cmdfont{twoside}~-- je zapnut oboustranný tisk, je nastaveno střídání
    okrajů a~umístění čísel stránek
    \item \cmdfont{draft}~-- je aktivován režim konceptu, jsou vyznačena místa
    s~přetečením textu (overfull boxes) a~je urychlen překlad
    \item \cmdfont{print}~-- je aktivován režim pro~tisk, jsou vynuceny černé odkazy
    v~textu (aby nesvítily modře), ale jsou ponechána barevná loga
    \item \cmdfont{bw}~-- dokument je sázen kompletně černobíle, loga jsou změněna
    na~černobílá, nadpisy i~odkazy jsou černé a~fakultní barva je nahrazena šedou
    \item \cmdfont{bwtitles}~-- nadpisy největších částí textu jsou sázeny černě,
    jinde je barva ponechána
    \item \cmdfont{noheader}~-- logo není umisťováno do~záhlaví stránek
    \item \cmdfont{margins}~-- okraje stránky jsou nastaveny podle normy~TUL
    \item \cmdfont{numbering}~-- je zapnuto číslování nadpisů (1., 1.1\dots)
    \item \cmdfont{fonts}~-- text dokumentu je sázen oficiálním písmem~TUL,
          více~v~části~\ref{pisma}
    \item \cmdfont{sfheadings}~-- pro~nadpisy kapitol a~sekcí není použito TUL~Mono,
    ale standardní bezpatkové písmo
\end{itemize}

\clearpage

Mimo~to je~možné ovládat zarovnání plovoucích prvků v~rámci dokumentu
(obrázky, tabulky, zdrojový kód).
Výchozím stavem balíku je~centrování obsahu i~popisků.
Následující~volby umožňují zarovnání doleva:

\bigskip

\begin{itemize}[label={}, leftmargin=0pt]
    \raggedright%
    \item \cmdfont{figuresleft}~-- obrázky i~jejich popisky jsou zarovnány doleva
    \item \cmdfont{tablesleft}~-- tabulky i~jejich popisky jsou zarovnány doleva
    \item \cmdfont{listingsleft}~-- popisky výpisů kódu jsou zarovnány doleva,
    zarovnání kódu se~naschvál nemění kvůli~čitelnosti
    \item \cmdfont{captionsleft}~-- popisky všech typů (obrázky, tabulky, kód)
    jsou hromadně zarovnány doleva, ale obsah je ponechán vycentrovaný
    \item \cmdfont{figcapsleft}~-- pouze popisky obrázků jsou zarovnány doleva
    \item \cmdfont{tabcapsleft}~-- pouze popisky tabulek jsou zarovnány doleva
    \item \cmdfont{lstcapsleft}~-- pouze popisky kódu jsou zarovnány doleva
\end{itemize}

\bigskip
\medskip

Dále lze nastavit různé~formy průběžného číslování~popisků. Je~možné je~buď přepnout
do stavu, kdy v číslování nezobrazují

\bigskip

\begin{itemize}[label={}, leftmargin=0pt]
    \raggedright%
    \item \cmdfont{singlecapnumbers}~-- je použito průběžné číslování v~celém dokumentu
    namísto číslování po~kapitolách
    \item \cmdfont{nocapnumbers}~-- je skryto označení typu a~čísla, je zobrazen pouze
    samotný text popisku
\end{itemize}
...
Sada příkazů umožňuje vkládat do~textu \textbf{loga} a~podobné grafické prvky.
Pro~úpravu jejich velikosti použijte příkazy \cmdnoindex{scalebox} nebo
\cmdnoindex{resizebox}. Loga neobsahují ochrannou zónu, příslušné volné místo
kolem nich musí zajistit autor dokumentu.

Pro~obecné konstrukce je k~dispozici několik příkazů, jejichž funkce závisí
na~volbách balíku. Vysází aktuální krátké či~dlouhé logo a~symbol TUL nebo té~její
...
tedy vysází „Faculty~of~Mechanical~Engineering“. Pomocí \cmd{CZTULsoucast}
a \cmd{ENTULsoucast} lze vysázet název aktuální části v~požadovaném jazyce.
Nejedná se o~grafické prvky,\linebreak ale o~běžná textová makra.
Do~této kapitoly jsem je zařadil vzhledem k~příbuznosti s~předchozími příkazy.
...
\begin{listing}
    \cmdnoindex{logo} \cmdnoindex{vspace\{1cm\}}
\end{listing}
...
neproporcionální písmo, které tvarově vychází ze~šablon, jimiž se
...
neproporcionální písmo \textbf{Noto Sans Mono}. To~je taky lepší volbou
pro~vypisování kódu (použito jako mono font v balíku \cmdfont{tulpackage/tul}).

\subsection{Instalace písem}

Všechna zmiňovaná písma jsou volně šiřitelná a~jsou zahrnuta do~distribuce
tohoto balíku. Najdete je v~adresáři \cmdfont{tulpackage/fonts}. Jsou ve~formátu OpenType,
který podporují jen novější implementace \LaTeX{}u: \XeLaTeX\ nebo Lua\LaTeX.
Starší pdf\LaTeX\ tento formát písem neumí.

Zmiňované implementace by s~moderními instalacemi \LaTeX{}u (pro~Windows doporučuji Mik\TeX)
měly fungovat tak~jak~jsou, bez~instalace na~vašem~počítači.
...
\cmdfont{fc-cache}. Přístup bez~instalace by~zde mohl také fungovat,
není to ale zatím otestováno.

\clearpage

\subsection{Použití písem}

Vzhledem k~tomu, že písma vyžadují překlad \XeLaTeX{}em nebo Lua\LaTeX{}em, balík
\cmdfont{tulpackage/tul} standardně do~písem nezasahuje bez vyžádání.
Jejich použití je potřeba aktivovat volbou \cmdfont{fonts} při vložení balíku.
...
\begin{listing}
    \cmdnoindex{usepackage[fonts,noheader]\{tulpackage/tul\}}\\
    \cmdnoindex{TULname\{Pavel Satrapa\}}\\
    \cmdnoindex{TULmail\{Pavel.Satrapa@tul.cz\}}\\
