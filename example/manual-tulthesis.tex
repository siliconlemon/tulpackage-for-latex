% tulpackage/example/manual-tulthesis.tex
% Obsahuje: Příklad použití třídy tulthesis z balíku tulpackage, návod ke psaní prací
% Tento dokument používá balíčky specifické pro XeLaTeX a lze jej přeložit pouze XeLaTeXem
...
\TULtitle{Třída tulthesis (\verze)}{tulthesis class (\verze)}
...
Návody tohoto typu poměrně často obsahují úvod do~\LaTeX{}u jako takového.
Já se~s~dovolením omezím jen na~popis samotné třídy \argument{tulthesis}
a~budu předpokládat, že jste v~otázkách \LaTeX{}u dostatečně zorientováni.
Pokud to není pravda, snad vám pomůže můj \emph{\LaTeX\ pro~pragmatiky}, který
najdete na~adrese:
...
\end{listing}
\listingcaption{Zahájení bakalářské práce na FM}
...
\end{listing}
\listingcaption{Zahájení diplomové práce na FS}
...
\end{verbatim}
\end{slisting}
\listingcaption{Příkazy pro nastavení metadat práce}
...
Následuje typicky abstrakt (či anotace) a~klíčová slova ve~dvou jazycích, pro~něž jsou
k~dispozici prostředí \prostredi{abstractCZ}, \prostredi{keywordsCZ},
\prostredi{abstractEN} a~\prostredi{keywordsEN} (případně ekvivalenty
\prostredi{annotationCZ} a~\prostredi{annotationEN}), poděkování (prostředí
\prostredi{acknowledgement}), obsah (standardní příkaz
\prikazneindex{tableofcontents}), seznam zkratek (prostředí \prostredi{abbreviations}) a
...
{
\begin{verbatim}
\documentclass[FM,DP]{tulpackage/tulthesis}
...
\end{slisting}
\listingcaption{Kompletní příklad struktury diplomové práce}


Dáváte-li přednost pdf\LaTeX{}u (což byste rozhodně neměli dělat, používejte raději
moderní implementace \XeLaTeX{} nebo \LuaLaTeX{}), změní se úvodní skupina příkazů na:
...
\end{verbatim}
\end{slisting}
\listingcaption{Nezbytná úprava preambule pro pdf\LaTeX}
...
\section{Vkládání obrázků a~tabulek}

Vkládání grafiky a~tabulek se v~třídě \argument{tulthesis} řídí běžnými zvyklostmi systému \LaTeX.
Pro~obrázky využijete balík \argument{graphicx} a~prostředí \prostredi{figure}, pro~tabulky prostředí \prostredi{table}.

Pokud chcete vynutit umístění plovoucího objektu přesně na~místo,
kde je uveden v~kódu (parametr \argument{[H]}), ujistěte se, že~máte načten balík \argument{float}.
Pro~kvalitní vodorovné linky v~tabulkách doporučujeme balík \argument{booktabs}.

Níže uvádíme příklad vložení obrázku.

\begin{slisting}
\begin{verbatim}
\begin{figure}[H]
    % \centering % (není nutné, řešeno možností balíku)
    \includegraphics[width=0.5\linewidth]
    {pictures/Liberec-TUL-Rektorat-1.jpg}
    \caption{Nový rektorát a~informační centrum}
    \label{fig:tul-rektorat}
\end{figure}
\end{verbatim}
\end{slisting}
\listingcaption{Kód pro vložení obrázku}

Výsledek sazby tohoto kódu bude vypadat následovně:

\vspace{0em}
\begin{figure}[H]
    \includegraphics[width=0.6\linewidth]{pictures/Liberec-TUL-Rektorat-1.jpg}
    \caption{Nový rektorát a~informační centrum}\label{fig:tul-rekt}
\end{figure}

Sazba~tabulek využívá standardní prostředí \prostredi{tabular}.
Pro~lepší čitelnost obsáhlejších tabulek je~vhodné zmenšit písmo příkazem \prikaz{small}.
V~kódu níže jsou vynechány opakující~se~řádky pro~přehlednost.
...
\begin{table}[H]
    ...
    \begin{tabular}{ll}
        ...
        \bottomrule
    \end{tabular}
    \caption{Základní informace...}\label{tab:zakladni_info}
\end{table}
\end{verbatim}
\end{slisting}
\listingcaption{Struktura kódu pro vložení tabulky}

Výsledná tabulka se zobrazí takto:

% \vspace{-0.6em}
\begin{table}[H]
    \caption{Základní informace o~Technické univerzitě v~Liberci}\label{tab:zakladni_informace}
    \small
    \begin{tabular}{ll}
        \midrule
        Název                               & Technická univerzita v~Liberci                                      \\
...
\chapter{Volby balíku}\label{volby}
...
\begin{description}
    ...
    pozdější překladu mohla vzniknout iluze, že práce vznikla v~pozdějším roce,
    než ve~skutečnosti.
...
\end{verbatim}
    \end{listing}
    \listingcaption{Definice vlastního typu práce}
...
\end{verbatim}
    \end{slisting}
    \listingcaption{Abstrakt na celou šířku stránky}

    Volba \argument{wide} je k~dispozici pro~všechna prostředí zajišťující
    sazbu abstraktu, anotace a~klíčových slov. Používejte ji konzistentně~-- u~všech nebo u
    žádného.
    ...
    \polozka{\prostredi{annotationCZ}} je prostředí pro~sazbu české anotace práce.
    Funguje zcela shodně jako \prostredi{abstractCZ}, namísto nadpisu „Abstrakt“ však vysází
    nadpis „Anotace“.

    \polozka{\prostredi{annotationEN}} je prostředí pro~sazbu anglické anotace práce.
    Funguje zcela shodně jako \prostredi{abstractEN}, namísto nadpisu „Abstract“ však vysází
    nadpis „Annotation“.
    ...
    \polozka{\prostredi{abbreviations}} je prostředí pro~sazbu seznamu zkratek. Obsah
...
\end{slisting}
\endgroup
\listingcaption{Ukázka sazby seznamu zkratek}
...
\end{verbatim}
\end{slisting}
\listingcaption{Konfigurace pro habilitační práci\label{lst:config}}
...
implementací, jako jsou \XeLaTeX{} nebo \LuaLaTeX{}), můžete sáhnout po~některém ze
...
Ve~verzi~2.1 bylo prostředí \argument{abbreviations} pro~seznam zkratek přepracováno
...
prostředí \argument{abbreviations} a~implementujte si seznam zkratek vhodnějším
...
\end{verbatim}
\end{slisting}
\listingcaption{Alternativní ruční tvorba seznamu zkratek}
