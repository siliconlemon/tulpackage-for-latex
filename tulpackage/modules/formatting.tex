...
\AtBeginDocument{%
    \ifthenelse{\boolean{TUL@italiccaps}}{%
...
% ==========================================================================
% Menší popisek pro uvádění zdrojů pod ilustracemi (Source Caption)
% ==========================================================================
\newcommand{\sourcecaption}[1]{%
    \par\vspace{2pt}%
    {%
        \footnotesize%
        \ifthenelse{\boolean{TUL@srccapsleft}}{%
            \raggedright%
        }{%
            \centering%
        }%
        \ifthenelse{\equal{\TUL@nguage}{EN}}{Source: }{Zdroj: }%
        #1%
        \par%
    }%
}%
...
\AtBeginDocument{%
    \ifthenelse{\equal{\TUL@nguage}{EN}}{%
        ...
        \setlength{\cftfignumwidth}{6.8em}%
        \setlength{\cfttabnumwidth}{6.3em}%
    }%
    \renewcommand{\listoffigures}{%
        ...
        {\hypersetup{linkcolor=\TUL@TOCtextcolor}\def\addvspace##1{}\@starttoc{lof}}\par\vspace{1em}%
    }%
    \renewcommand{\listoftables}{%
        ...
        \vspace{0.35em}\thispagestyle{plain}%
        {\hypersetup{linkcolor=\TUL@TOCtextcolor}\def\addvspace##1{}\@starttoc{lot}}\par\vspace{1em}%
    }%
}%
...
\ifcsdef{abbreviations}{}{%
    \newenvironment{abbreviations}{%
        ...
        \noindent\begin{tblr}[long, entry=none]{
...
\providecommand{\listoflistings}{%
    ...
    \ifthenelse{\equal{\TUL@nguage}{EN}}{%
        ...
        \section*{\LARGE Seznam výpisů kódu}%
        \@mkboth{Seznam výpisů kódu}{Seznam výpisů kódu}%
        \addcontentsline{toc}{chapter}{Seznam výpisů kódu}%
    }%
    \vspace{0.85em}\thispagestyle{plain}%
    {\hypersetup{linkcolor=\TUL@TOCtextcolor}\def\addvspace##1{}\@starttoc{lol}}\par\vspace{1em}%
}%
...
% --- Popisek výpisu ---
\providecommand{\TUL@listing@prefix}{Výpis}%
\AtBeginDocument{%
    \ifthenelse{\equal{\TUL@nguage}{EN}}{%
        \renewcommand{\TUL@listing@prefix}{Listing}%
    }{%
        \renewcommand{\TUL@listing@prefix}{Výpis}%
    }%
}%
...
\providecommand{\listingcaption}[1]{%
    ...
    \ifthenelse{\boolean{TUL@lstcapsleft}}{%
        {\raggedright\noindent%
            \ifthenelse{\boolean{TUL@italiccaps}}{\itshape}{}% % chktex 6
            \ifthenelse{\boolean{TUL@nocapnumbers}}{%
                #1%
            }{%
    ...
    \par
    \def\@sourceafterlabel\label##1{\label{##1}
        \@ifnextchar\sourcecaption{\vspace{-6pt}}{\vspace{8pt}}
    }%
    \@ifnextchar\label{\@sourceafterlabel}{\@ifnextchar\sourcecaption{\vspace{-6pt}}{\vspace{8pt}}}%
}%
...
\newenvironment{listing}{%
    ...
    \def\FrameCommand##1{%
        \begin{tikzpicture}[baseline=0pt]%
            \node[fill=tullightgray, rounded corners=3.5pt, inner xsep=10pt, inner ysep=5pt, outer sep=0pt] {##1};%
        \end{tikzpicture}%
    }%
    \MakeFramed{\advance\hsize-20pt\linewidth=\hsize\TUL@FrameRestore}%
}{%
    \endMakeFramed%
    \par\addvspace{1pt}%
}%
...
\newenvironment{slisting}{%
    ...
    \footnotesize%
    \linespread{1.0}\selectfont
    \color{black}\raggedright\sloppy%
    \setlength{\FrameSep}{0pt}%
    \setlength{\FrameRule}{0pt}%
    \setlength{\OuterFrameSep}{0pt}%
    \def\FrameCommand##1{%
        \begin{tikzpicture}[baseline=0pt]%
            \node[fill=tullightgray, rounded corners=3.5pt, inner xsep=10pt, inner ysep=0pt, outer sep=0pt] {##1};%
        \end{tikzpicture}%
    }%
    \MakeFramed{\advance\hsize-20pt\linewidth=\hsize\TUL@FrameRestore}%
}{%
...
\newenvironment{linelisting}{%
    ...
    \setlength{\FrameRule}{0pt}%
    \setlength{\OuterFrameSep}{0pt}%
    \def\FrameCommand##1{\hspace{16pt}{\color{\tulcolor}\vrule width 1.0pt}\hspace{14pt}##1}%
    \MakeFramed{\advance\hsize-31pt\linewidth=\hsize\TUL@FrameRestore}%
}{%
    \endMakeFramed%
    \par\addvspace{6pt}%
}%
