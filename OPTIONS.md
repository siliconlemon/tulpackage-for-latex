...
- **print** – Režim pro **barevný tisk**.
  ...
  - **Vynutí černé odkazy** v textu a obsahu (aby v tisku nesvítily barevné hyperlinky).
...
- **sfheadings** – Nastaví bezpatkové písmo pro nadpisy (pokud není zvolena možnost *fonts*, 
  použije se standardní Sans Serif).
...
- **singlecapnumbers** – Průběžné číslování v celém dokumentu (Obrázek 1, Obrázek 2...) 
  namísto číslování po kapitolách (Obrázek 1.1, 1.2...).
- **nocapnumbers** – Skryje označení typu a čísla (např. zmizí "Obrázek 1:"), zůstane pouze samotný text popisku.
- **italiccaps** – Nastaví text popisků **kurzívou**.
...
- **print** – Mode for **color printing**.
  ...
  - **Forces black links** in text and content (so colorful hyperlinks do not appear in print).
...
- **singlecapnumbers** – Continuous numbering throughout the document 
  (Figure 1, Figure 2...) instead of numbering by chapters (Figure 1.1, 1.2...).
- **nocapnumbers** – Hides the label and number (e.g., "Figure 1:" disappears), 
  only the caption text remains.
- **italiccaps** – Sets caption text to **italics**.
